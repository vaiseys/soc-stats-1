...
```{r}
#| echo: false
gss2022 <- readRDS(here("data", "gss2022.RDS"))
glimpse(gss2022)
```
...
We can do these things without simulation. Here is the analytic test for a difference in proportions, where $\hat{p}$ is just the observed whole sample proportion.
...
We have most of the values we need from above already. But here are two more, the formula, and the results.
...
zstat <- 
  (p1 - p2) /
  sqrt(pooled_prop * (1 - pooled_prop) * (1 / n1 + 1 / n2))
...
As we will very briefly discuss, people often use something called a **t-test** for comparing differences in means. For now, we're going to stick with our approach based on z-scores and the normal distribution. The formula is:
...
The z-statistic is `r zstat` and the p-value is `r pval`. Looks like the difference in the population probably isn't exactly zero!
...
The simplest social science questions are about _comparing groups_. Comparing means and proportions across just two groups are the simplest versions of this but give you all the basic ideas. The only real difference between the binary and continuous cases is, once again, that the SD is its "own thing" in the continuous case rather than a function of the central tendency, as in the Bernoulli case.

## Homework

Compare two groups on two binary and two continuous variables. Do hypothesis tests and confidence intervals. State the null hypotheses and the alternatives. To receive full credit, don't use the same grouping variable for all four; use at least two different grouping variables.
