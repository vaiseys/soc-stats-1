...
1. The **Law of Large Numbers** states that repeated _random_ observations wil converge to the true value;
...
:::fragment
Think about it: what would it mean if it were true that $P(\text{College}) \times P(\text{Very happy}) = P(\text{College} \cap \text{Very happy})$?
:::
...
pt <- d_num |> 
  group_by(college, vhappy) |> 
  summarize(joint_prob = n() / nrow(d_num))

tt(pt, digits = 3)
```

If the joint probabilities are the same as the products of the marginal probabilities, then two variables are _independent_.
...
For example, what is the probability of being very happy _conditional_ on having a college degree? What is the probability of being very happy _conditional_ on NOT having a college degree?

:::fragment
These values are `r round(mean(d_num$vhappy[d_num$college==1]), 3)` and `r round(mean(d_num$vhappy[d_num$college==0]), 3)`, respectively. Again, if these were the same, the two variables would be _independent_.
:::
...
ggplot(cp,
  ...
  labs(x = "",
       y = "P(Very happy)",
       title = "Happiness by level of education",
