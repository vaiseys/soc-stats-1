...
The **generalized linear model** takes the relationship between x and y and makes it _linear_ and _additive_. That is, it expresses the relationship between $Y$ and $X$ such that $\alpha$ is a _point_ and $\beta$ is a _distance_.

The **linear predictor** is an additive function of $\alpha$, $\beta$, and $x_i$, usually $\alpha + \beta x_i$. 

## GLM link function

The left-hand side (e.g., $\log P(y_i=1)$ determines the **link function**.) For example, the left-hand side of the t-test as linear model we saw above is just $y_i = \dots$ 

This is called the **identity function** because $f(\mathbb{E}(Y)) = \mathbb{E}(Y)$. We will see other functions soon.

## GLM family

The other part of the GLM is the **family**. This is the _probability distribution_ we are using for the model. Above, we used the _normal_ (or Gaussian) distribution. This helps us determine the _maximum likelihood_ values of the parameters. 

Don't worry too much about the details of this for now. Just remember that the type of GLM is determined by _link_ and _family_.

## GLMs for two-group comparisons
...
table(dgrad$pargrad, dgrad$grad) # rows are levels of pargrad
```
...
$$
\mathbb{E}(\text{grad}_i) = \alpha + \beta(\text{pargrad}_i)
$$
...
:::{.callout-note .absolute style="top: 75%; width: 100%;"}
Here $\mathbb{E}(y_i)$ means "the expected value of $y_i$."
:::
...
[**Link**: identity]{.fragment}

[**Family**: binomial (Bernoulli)]{.fragment}

## Using a custom R function

Let's keep the output display simple for this section by creating a simple version of `tidy()`.
...
:::{.callout-note .absolute style="top: 65%;"}
Don't worry right now about were the standard errors come from. You probably get the general idea from before but we will talk about that a bit later.
:::
...
What is the link? What is the family? What do these numbers mean?
...
[Remember that $\beta$ is a _distance_ and that $\alpha$ is a _point_.]{.fragment}

## Risk ratio

This is just the ratio of conditional probabilities, $P(Y=1 \mid X = x)$.
...
:::fragment
```{r}
py_x1 <- mean(dgrad$grad[dgrad$pargrad == 1])
py_x0 <- mean(dgrad$grad[dgrad$pargrad == 0])
...
Which one of these is properly written as a GLM?
...
::: fragment
What is $\beta$? Consider this...

```{r}
exp(mod3$coefficients["pargrad"])
```
:::

::: fragment
$\beta$ here is a **log risk ratio**.
:::
...
:::{.callout-note .absolute style="top: 60%;"}
The two odds ratios remain the same regardless of the coding of $Y$ and $X$ or which is considered the outcome vs. the predictor. This makes them attractive when there is no "natural" value for 0 vs. 1 (e.g., Democrat vs. Republican) and/or no "natural" predictor.
:::
...
:::{.callout-note .absolute style="top: 60%;"}
The log odds ratio is always the same number (except for a change in sign) regardless of the coding of $Y$ and $X$ or which is considered the outcome vs. the predictor. So odds ratios of 4 and .25 become log odds ratios of 1.39 and -1.39.
:::


## Binary GLM summary {.smallish}

|Term             |Identity              |Log              |Logit           |
|-----------------|----------------------|-----------------|----------------|
| $\alpha$        | probability          | log prob.       | log odds       |
| $\beta$         | diff. in prob.       | log risk ratio  | log odds ratio |

:::{.callout-tip .absolute style="top: 60%;"}
Linear probability models (identity link) and logistic regression models (logit link) are both common in sociology. The main value of the former is **simplicity** and the main value of the latter is **realism**. As we'll see later, sometimes the LPM can create predicted probabilities below 0 or above 1, which is suboptimal!
:::

# Likelihood: a digression {.inverse}

## Maximum likelihood

We have learned what the units are of $\alpha$ and $\beta$, but let's talk about how we get these numbers, which is related to the idea of **maximum likelihood**.

## Probability

When we talk about the probability of an event, this is inherently _forward looking_. 

For example, what is the probability of getting exactly two heads on three flips of a fair coin (i.e., when the coin's probability of flipping heads is $\pi = .5$)?

[This probability is $3/8$, or 37.5 percent.]{.fragment}

## Likelihood

Likelihood, by contrast, is _backward looking_. 

It starts with events that have already happened and reasons back to the parameters that generated them. For example, what is the _most likely_ value of $\pi$ for a coin that actually flipped heads two out of three times?

[The _most likely_ value of $\pi$ in this case is $2/3$ or 66.7%.]{.fragment}

## Probability vs. likelihood

We ask different conditional questions depending on **what we have** and **what we don't**.

For probability, we ask: _What is the probability of a data distribution **given** the model_? 

For likelihood, we ask: _What is the likelihood of a model **given** a data distribution_?

## Maximum likelihood estimates

We've already been using likelihood without realizing it. For example, if we find that 560 people out of 1000 support abortion rights in our data, we infer that $p = .56$ is the _most likely_ value of the population parameter.
...
ggplot(bd,
  ...
  scale_x_continuous(breaks = 0:10) +
  theme_light() +
  labs(x = "k (# of \"successes\")",
       y = expression(paste("P(X = k) when ", pi, " = .7")))

```

This is a probability distribution, like others we have seen, with given parameters ($\pi = .7$  and $n = 10$). It is _forward looking_ in the sense that it shows the probability of what might happen given those parameters.

## Likelihood {.smallish}

Now reverse it: we _actually saw_ 7 out of 10 "successes." What is the _likelihood_ of seeing 7 _if the value of $\pi$ is actually .7_?

```{r}
dbinom(x = 7, size = 10, p = .7)
```

:::fragment
But wait! What if the actual value of $\pi$ is .65? How likely is it that we'd see 7 out of 10 successes?

```{r}
dbinom(x = 7, size = 10, p = .65)
```

There's not much difference. In fact, $\pi = .7$ is only about 5.8% _more likely_ to have produced the observed data than $\pi = .65$.
:::
...
:::column
To find the MLE of our data (7 out of 10 successes), we can do a fancy version of "guess and check" called **grid approximation**. Let's check the likelihood of seeing 7 out of 10 for all the values of $\pi$ in increments of .05.
:::
...
Would the likelihoods be different in these situations?
...
These **likelihood distributions** can be used to construct confidence intervals, though they rarely are used directly in that way in practice. But the principle of MLE and how it related to sample size should be pretty clear from this exercise.

## Likelihood is rowwise!

Check out `extras\rowwise-likelihoods.R` for a demo.

# Count models {.inverse}
...
The simplest distribution to deal with count data is the **Poisson distribution**. This distribution has one parameter, $\lambda$, which determines the mean ($\lambda$) and the variance (also $\lambda$).

## Three values of $\lambda$

```{r}
#| echo: false

pd <- expand_grid(
  lambda = c(1.2, 2.3, 3.5),
  k = 0:12) |> 
...
Take a moment. What's the probability of getting exactly 3 visitors to a coffee shop in an hour if $\lambda = 3$ visitors per hour? [(It's about 22.4%.)]{.fragment}

## Poisson GLM

Poisson GLMs use the _log link_ and the _Poisson distribution_. The basic formula looks like this:

$$
  \log(y_i) = \alpha + \beta x_i
$$

Thus, for each respondent $i$, $\lambda_i = \alpha + \beta x_i$.

## TV hours {.smallish}

Let's model TV hours as a function of having a college degree for those who are employed full time. First, here's the distribution.

```{r}
#| echo: false

d <- gss2022 |> 
  select(tvhours, degree, wrkstat) |>
  drop_na() |>
  filter(wrkstat == 1) |> 
...
## Estimating the Poisson GLM

```{r}
poismod <- glm(tvhours ~ college,
               data = d,
...
gss2022 |> 
  select(sibs, attend) |> 
  drop_na() |> 
  mutate(church = if_else(attend >= 7, "weekly", "less")) |> 
  summarize(numsibs = mean(sibs), .by = church) |> 
  tt(digits = 3)
...
:::fragment
$$
  \log(sibs_i) = 1.20 + .174(weekly_i)
$$
:::

:::{.fragment .callout-tip .absolute style="top: 65%;"}
## Challenge\!

According to this model, what is the probability that someone with (and without) a college degree watches 4 or more hours of TV per day?
:::

```{r}
#| include: false

1 - (dpois(0:3, 3.31) |> sum()) # BA
1 - (dpois(0:3, 3.94) |> sum()) # < BA
```
...
# Three or more groups {.inverse}

## Refresher: two groups

The comparisons we've seen so far are of two groups, which we have generally coded as $X=0$ and $X=1$ (e.g., college degree or not). So, for a difference in means (a.k.a. a t-test) we have the formula:

$$
\begin{align}
  y_i &\sim \cal{N}(\mu_i, \sigma) \\
  \mu_i &= \alpha + \beta x_i
\end{align}
$$
...
## Three groups {.smallish}

The logic is similar when we have three groups. Imagine we have the personal incomes of three groups: less than high school, high school, college.
...
Writing these three means as an equation, we get:

$$
\begin{align}
  y_i &\sim \cal{N}(\mu_i, \sigma) \\
  \mu_i &= \alpha + \beta_1 (HS_i) + \beta_2(BA_i)
\end{align}
$$

Now the "no degree" group is the reference category. 

## Initial results {.smallish}

Here are the results.

```{r}
inc3mod <- lm(realrinc ~ edlvl, data = incdata)
mytidy(inc3mod)
```

Uh oh! We got a different reference category from what we planned. With a `<chr>` variable, R will pick the alphabetically lowest value as the reference category. We can change that be making it an **ordered factor**.
...
incdata <- incdata |> 
  mutate(edfac = factor(
    edlvl,
    levels = c("no degree", "high school", "BA+"), # actual values
    labels = c("none", "HS", "BA+"), # will be shown
    ordered = TRUE
  )
  )
...
## More results {.smallish}

```{r}
inc3mod <- lm(realrinc ~ edfac, data = incdata)
mytidy(inc3mod)
```

This is annoying because when we have an ordered factor, R defaults to "polynomial contrasts," which you _really_ don't need to worry about (`.L` means "linear", `.Q` means "quadratic", etc.). You can solve this by asking for "treatment contrasts" as follows:

```{r}
contrasts(incdata$edfac) <- "contr.treatment"
```


## Final results {.smallish}
...
Note that we could use any reference category we want. The reference value is $\alpha$, which is always a _point_ and is the expected value of the omitted (or reference) group. The $\beta$ values are _distances_ from that reference point to the mean of the relevant group. This is the same logic as two groups!

## Same for all GLMs

```{r}
inc3pois <- glm(round(realrinc, 0) ~ edfac, # rounding to nearest dollar
                data = incdata, 
                family = poisson(link = "log"))
mytidy(inc3pois)
```

The fundamental ideas are no different outside of the identity/Gaussian GLM. The alpha is still a point and the betas are still distances, just in different units.

## Joint significance {.smaller}

One tricky thing with three or more groups is that we can't look at a single beta coefficient to tell if the group differences are significantly different from zero. We can compare the fit of the **null model** to the fit of the **fitted model**. The null hypothesis here is that $\beta_1 = \beta_2 = 0$ (or that all groups have the same mean).

```{r}
inc3onemean <- lm(realrinc ~ 1, data = incdata) # null model
anova(inc3onemean, inc3mod)
```

This result be exactly the same regardless of which category is chosen as the reference.

:::callout-tip
Don't worry about the F-statistic or sum of squares here. This is for **one-way ANOVA** (analysis of variance). As you'll see later, this type of comparison generalizes far beyond one type of model.
:::

# Numeric predictors {.inverse}

## Groups {.smallish}

So far on the right-hand-side (RHS) of our models, we have had **indicators** of (or **dummy variables** representing) one, two, or three expectations related to discrete groups. Extending this to more groups is pretty easy.

:::fragment
We might keep extending the logic of group comparison. `degree`, for example, has 5 distinct levels in the GSS. But why stop there? The `educ` variable has (up to) 21 levels!
:::

## A bridge too far {.smaller}
...
```{r}
edmod1 <- lm(realrinc ~ factor(educ), # dummies
             data = incdata)
mytidy(edmod1)
```
...
# set up prediction vector (valid levels of education in data)
preds <- tibble(
  educ = unique(incdata$educ)
) |> 
  arrange(educ)

# get predictions from the model
preds$yhat <- predict(edmod1, newdata = preds)
...
- from discrete means to lines
- functional form ("multinomial", stepwise, linear)
- OLS as ML estimator (old slides; don't do matrix algebra)
- model selection with LRT
  - saturated vs. restricted models

- application of linear fits to GLMs (e.g., "linear in the logit")
- two binary predictors (with -> without interactions)
- model selection again

- correlation is scaled bivariate regression
- tabular measures of association as count models
